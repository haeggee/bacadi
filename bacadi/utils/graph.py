...
from bacadi.exceptions import InvalidCPDAGError
...
def random_consistent_expansion(*, key, cpdag, force_expansion_with_new_vee_structure=False):
    """
    ...
    if jnp.sum((cpdag == cpdag.T) & (cpdag == 1)) == 0:
        return cpdag
    ...
    while n_left > 0:
        ...
        if not found_any_valid_candidate:
            if force_expansion_with_new_vee_structure:
                # pick a random sink `sk` from potential sinks
                assert sinks, f"A sink should always exist in a PDAG but we didn't find one"
                key, subk = random.split(key)
                sk = random.choice(subk, sinks)

                # follow steps in `if i_valid_candidate` (orient edges towards sk, delete sk and adjacent egdes to sk)
                # this creates a new v-structure at sk, but an unavoidable one if we want to obtain some DAG
                undirected_neighbors_sk = (A[sk, :] == 1) & (A[:, sk] == 1)
                G[sk, onp.where(undirected_neighbors_sk)] = 0
                A[sk, :] = 0
                A[:, sk] = 0
                node_exists[sk] = 0
                n_left -= 1
                ...
                err_msg = (
                        'Unable to create random consistent extension of CPDAG because non-chordal: ' + adjmat_to_str(
                    cpdag) +
                        ' | G: ' + adjmat_to_str(G) +
                        ' | A: ' + adjmat_to_str(A) +
