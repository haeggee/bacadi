...
pc_wrapper <- function(data,systemVars,contextVars,m.max=Inf,known,targets=NULL,alpha=1e-2,verbose=0,subsamplefrac=0.0,test='gaussCIcontexttest',obsContext=matrix(0,1,length(contextVars))) {
  # data:          Nxp matrix
  ...
  cpdag <- pc_modified(labels=colnames(data),suffStat,indepTest,systemVars,contextVars,alpha=alpha,verbose=verbose,fixedGaps=fixedGaps,fixedEdges=fixedEdges, u2pd="relaxed", known=known, targets=targets, m.max=m.max)
  cpdag <- cpdag@graph
