source('baselines/jci/pc_wrapper.R', chdir=T)
library(methods)
library(pcalg)
...
result <- pc_wrapper(data=dataset, systemVars=1:d, contextVars=(d+1):p, alpha='{ALPHA}', obsContext=matrix(0,1,r), test='{INDEP_TEST}', known='{KNOWN}', targets=targets, m.max='{MMAX}')
show("pc.R")
show(result$cpdag)
